    ...
    if ([aTask.name isEqualToString:@""] || !aTask.name) {
        ...
        if ([[KBNTaskListService sharedInstance] hasCountLimitBeenReached:aTaskList]){
            ...
            if ([KBNReachabilityUtils isOnline] &&
                ...
                [self.dataService createTaskWithName:aTask.name taskDescription:aTask.taskDescription order:aTask.order projectId:aTaskList.project.projectId taskListId:aTaskList.taskListId priotity:aTask.priority completionBlock:^(NSDictionary *params) {
